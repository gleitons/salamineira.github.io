...
function criadorDePages(cod) {
    ...
    const itemPopular = cod
    const codV = contrato[cod].imovel
    console.log(contrato[cod].imovel)
    

    const infoDaMatricula = imovel[codV].matricula.length > 0 ? `original da matrícula ${imovel[codV].matricula}, registrado em ${imovel[codV].registrodia}/${imovel[codV].registromei}/${imovel[codV].registroAno}, sendo este contrato de parte de uma` : ''
    
    console.log(contrato[cod].valor)

    const infoDoImovel = imovel[contrato[codV].imovel]
    ...
    document.querySelector('#pageOne').innerHTML = `<h2>CONTRATO PARTICULAR DE COMPRA E VENDA DE IMÓVEL RURAL</h2>

<p>Contrato de compra e venda de <strong>imóvel rural</strong> que entre si fazem <span id="topoCompradores">os vendedores (a)</span> e <span id="topoVendedores">compradores (a)</span> nas seguintes condições:</p>
...
<p><strong>CELEBRAM</strong> este ato um contrato de compra e venda de <strong>direito de posse de terreno rural</strong>, ${infoDaMatricula} <strong>área de ${infoDoImovel.areatotalimovel} M² (${infoDoImovel.areatotalimovel.extenso()} metros quadrados)</strong>, sendo <strong>${nomeaRua(infoDoImovel.medidafrente)} lineares de frente</strong> pra ${infoDoImovel.nomefrente}, ${nomeaRua(infoDoImovel.medidafundo)} lineares de fundos pra ${infoDoImovel.nomefundo}, ${nomeaRua(infoDoImovel.medidadireito)} lineares do lado direito pra ${infoDoImovel.nomeDireito} e ${nomeaRua(infoDoImovel.medidaEsquerda)} lineares com lado esquerdo pra ${infoDoImovel.nomeesquerdo}. Localizado em<strong> ${enderecoImovelLote(infoDoImovel.endereco,infoDoImovel.numero,infoDoImovel.bairro, infoDoImovel.cep, infoDoImovel.cidade, infoDoImovel.estado)}</strong>. ${infoDoImovel.denominacao}.
...
observContrato.textContent = infoDoImovel.observacao != null ? infoDoImovel.observacao : ''
    ...
    for (let i = 0; i < contrato[cod].compradores.length; i++) {
        ...
        if (prop1 != '') {
            ...
            var principalC = `<strong id="compPrint">${pessoal[prop1 - 1].nome.toUpperCase()}</strong>, ${pessoal[prop1 - 1].nacionalidade}, ${pessoal[prop1 - 1].estadoCivil}, ${pessoal[prop1 - 1].profissao}, <strong>CPF: ${pessoal[prop1 - 1].cpf}</strong>, Identidade de número <strong>${pessoal[prop1 - 1].identidade.toUpperCase()}</strong> residente e domiciliado na ${pessoal[prop1 - 1].endereco}, Nº ${pessoal[prop1 - 1].numero}, bairro ${pessoal[prop1 - 1].bairro}, CEP: ${pessoal[prop1 - 1].cep}, município de ${pessoal[prop1 - 1].cidade} - ${pessoal[prop1 - 1].estado}`
    ...
    observContrato.innerHTML = contrato[cod].observacao
...
function carregaCFundo() {
    ...
    for(let i = 0; i < 4; i++){
        ...
        if(fontesACarregar == 1) {
            location.href = `./print-rural${todasFontes[1]}.html#${enderecoLink}`
        } else if (fontesACarregar == 2) {
            location.href = `./print-rural${todasFontes[2]}.html#${enderecoLink}`
        } else {
            location.href = `./print-rural${todasFontes[3]}.html#${enderecoLink}`
        }        
        //carrF[i].style.fontSize = todasFontes[fontesACarregar]
    }
}
...
function enderecoImovelLote(endereco, numero, bairro, cep, cidade, estado ) {
    return `${endereco.toUpperCase()}, Nº ${numero}, ${bairro.toUpperCase()}, ${cep}, ${cidade} - ${estado}`
}
